...
const modelValue = defineModel<VannaSqlConfig>({ required: true });

// --- FUNCTIONS ---
function updateField(field: keyof VannaSqlConfig, value: string | number | null) {
  if (field === 'port') {
    modelValue.value = { ...modelValue.value, [field]: Number(value) || 1433 };
  } else {
