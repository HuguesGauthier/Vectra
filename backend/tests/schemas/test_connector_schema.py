...
def test_connector_validation_cron():
    # Use valid connector_type "local_file"
    with pytest.raises(ValidationError) as exc:
        ConnectorCreate(name="Fail", connector_type="local_file", schedule_cron="invalid cron")
    assert "Invalid cron" in str(exc.value)

    # Valid cron
    c = ConnectorCreate(name="OK", connector_type="local_file", schedule_cron="*/5 * * * *")
...
def test_connector_config_size():
    ...
    with pytest.raises(ValidationError) as exc:
        ConnectorCreate(name="Fail", connector_type="local_file", configuration=big_dict)
