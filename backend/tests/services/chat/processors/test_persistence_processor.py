...
async def test_user_persistence_hot_and_cold(mock_context):
    ...
    with patch("app.services.chat.processors.persistence_processor.ChatPostgresRepository") as MockRepo:
        ...
        mock_context.chat_history_service.add_message.assert_awaited_once_with("test-session", ROLE_USER, "Hello")
...
async def test_assistant_persistence_call_flow(mock_context):
    ...
    with patch("app.services.chat.processors.persistence_processor.ChatPostgresRepository") as MockRepo:
        ...
        call_args_list = mock_context.metrics.record_completed_step.call_args_list
        # We invoke this method likely more than once?
        # Actually in this processor it's called once.
        # But let's check if the specific step type was recorded.

        found = False
        for call_args in call_args_list:
            if call_args.kwargs.get("step_type") == "assistant_persistence":
                found = True
                break

        assert found, "Metrics for assistant persistence not recorded"
