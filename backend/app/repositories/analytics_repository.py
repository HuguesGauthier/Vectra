...
class AnalyticsRepository:
            ...
            query = text(
                ...
                ORDER BY 
                    CASE step_key
                        ...
                        WHEN 'cache_lookup' THEN 2
                        WHEN 'query_rewrite' THEN 3
                        WHEN 'vectorization' THEN 4
                        WHEN 'retrieval' THEN 5
                        WHEN 'reranking' THEN 6
                        WHEN 'synthesis' THEN 7
                        WHEN 'persistence' THEN 8
                        WHEN 'cache_update' THEN 9
                        WHEN 'trending' THEN 10
                        ELSE 99
                    END ASC
            """
            )
            # Note: Removed hardcoded CASE ordering for performance/simplicity, Service can sort if needed.
    ...
    async def get_topic_frequencies(self, assistant_id: Optional[UUID] = None) -> List[Any]:
        ...
        result = await self.db.execute(query)
        return result.all()  # list of Row(frequency)
    ...
    async def get_session_counts(self, cutoff: datetime) -> List[Any]:
        ...
        # Using raw SQL for the nested aggregation
        query = text(
            """
    ...
    async def get_user_usage_stats(self, cutoff: datetime, limit: int = 10) -> List[Any]:
        ...
        Joins UsageStat with User to get names.
        """
        from app.models.user import \
            User  # Late import to avoid circular dependency
